...
FtpContext::FtpContext(asio::io_service& ios, const std::string& ip, unsigned short port, const std::string& user,
	...
	: _ios(ios), _ip_address(ip), _port(port), _user(user), _pwd(pwd), _dir(dir), _fileList(fileList), _ctrlSession(nullptr), _dataSession(nullptr), _inFile(_ios), _outFile(_ios)
{
...
void FtpContext::ReBuild(State & s)
{
	...
	_ctrlSession->Timer().expires_from_now(std::chrono::seconds(30), ec);
	if (ec)
		return;
