...
public:
    FtpContext(boost::asio::io_service &ios, const std::string &ip, unsigned short port, const std::string &user,
               const std::string &pwd, const std::string &dir,
               std::shared_ptr<threadsafe_queue<std::string> > fileList);
    ...
    const FtpSession *GetCtrlSession() const;

    const FtpSession *GetDataSession() const;
    FtpSession *GetCtrlSession();
    FtpSession *GetDataSession();
...
inline const FtpSession* FtpContext::GetCtrlSession() const {
    return _ctrlSession.get();
}

inline const FtpSession* FtpContext::GetDataSession() const {
...
inline FtpSession *FtpContext::GetCtrlSession() {
    return _ctrlSession.get();
}

inline FtpSession *FtpContext::GetDataSession() {
    return _dataSession.get();
}
